            ...
            case "loginAcc":
                ...
                if (accdao.checkAccount(username, password) == null) {
                    ...
                    if (accdao.checkAccount(username, password).getRole() == 3) {
                      ...
                      if (accdao.checkAccount(username, password).getRole() == 1) {
                        session.setAttribute("account", accdao.checkAccount(username, password));
                        session.setAttribute("listOrderForShipper",indao.getlistOrderForShipper() );
                        Account shipper = accdao.checkAccount(username, password);
                        session.setAttribute("listHistoryOfShipper", indao.getHistoryOfShipper(shipper.getUsername()));
                        response.sendRedirect("ForShipper.jsp");
